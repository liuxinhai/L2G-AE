...
grouping_module=tf.load_op_library(os.path.join(BASE_DIR, 'tf_grouping_so5.so'))
...
def query_ball_point1(radius1, radius2, nsample, xyz1, xyz2):
    '''
    Input:
        radius: float32, ball search radius
        nsample: int32, number of points selected in each ball region
    ...
    Output:
        ...
        pts_cnt: (batch_size, npoint) int32 array, number of unique points in each local region
    '''
    #return grouping_module.query_ball_point1(radius, nsample, xyz1, xyz2)
    return grouping_module.query_ball_point1(xyz1, xyz2, radius1, radius2, nsample)
ops.NoGradient('QueryBallPoint1')
...
def knn_point_list(k_list, xyz1, xyz2):
    '''
    Input:
        k: int32, number of k in k-nn search
        xyz1: (batch_size, ndataset, c) float32 array, input points
    ...
    for i in range(len(k_list)):
        # print dist, k
        outi, out = select_top_k(k_list[i], dist)
        idx = tf.slice(outi, [0,0,0], [-1,-1,k_list[i]])
        val = tf.slice(out, [0,0,0], [-1,-1,k_list[i]])
        val_list.append(val)
        idx_list.append(idx)
    # print idx, val
    #val, idx = tf.nn.top_k(-dist, k=k) # ONLY SUPPORT CPU
